  ...
  return (
    <div className="relative" ref={dropdownRef} data-tutorial="language-selector">
      ...
      <button
        ...
        className="hidden md:flex p-2 hover:bg-gray-100 rounded-lg transition-colors text-[#8E7F71] hover:text-[#4A3728] items-center gap-1"
      ...
      >
        ...
        <span className="text-sm text-[#4A3728] font-medium flex-1">
          {LOCALE_NAMES[locale]}
        </span>
        <svg className="w-5 h-5 text-[#8E7F71]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {isOpen && (
        <div className="absolute right-0 md:mt-2 mt-1 w-full md:w-40 bg-white border border-[#E5E7EB] rounded-lg shadow-lg overflow-hidden z-50">
          {locales.map((loc) => (
            <button
              key={loc}
