...
export default function LanguageSelectorProfile({ initialLanguage = 'en' }: LanguageSelectorProfileProps) {
  ...
  return (
    <div className="bg-[#111] border border-white/10 rounded-2xl p-6 transition-colors hover:border-white/20" data-tutorial="language-selector">
      <div className="flex items-center gap-3 mb-6">
        <div className="p-3 bg-purple-500/20 rounded-xl border border-purple-500/30">
          <Globe className="w-6 h-6 text-purple-400" />
        </div>
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-white">{t('preferences.language')}</h3>
          <p className="text-sm text-gray-400">{t('preferences.selectInterfaceLanguage')}</p>
        </div>
      </div>
